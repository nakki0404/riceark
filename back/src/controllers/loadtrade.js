...
if (existingDoc) {
  ...
  const updatedDocument = await trading_data.findOneAndUpdate(
    filter,
    { $set: { Stats: existingDocStats.sort((a, b) => new Date(b.Date) - new Date(a.Date)) } },
    { new: true }
  );
