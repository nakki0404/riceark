...
const listsReducer = (state = initialListsState, action) => {
  switch (action.type) {
    case ADD_LIST:
      ...
      if (state.lists.some(item => item.Title === action.payload.Title)
...
const reportsReducer = (state = initialReportsState, action) => {
  switch (action.type) {
    case ADD_REPORT:
      if(state.reports.find(item=>item.Title===action.payload.Title&&item.Body===action.payload.Body))
      return { ...state };
    ...
    case DELETE_REPORT:
      return {
        ...state,
        reports: state.reports.filter((item) => item.Title !== action.payload.Title),
      };
    default:
      return state;
  }
